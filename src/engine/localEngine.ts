...
import { cabtObservationToGameView, projectDecision } from '../lib/cabt/cabtProjection';
import { buildLiveSteps, createLiveStepState, type LiveStepState } from './liveSteps';
...
import type { EngineResponse, GameView, SeatView } from '../lib/game/types';
...
export class LocalEngineController {
  ...
  // Everything that turns bridge responses into playback steps — the canonical
  // log stream, concealment, timeline ids, and both frame buffers. Owned by
  // liveSteps.ts so tests can drive the identical path offline.
  private live: LiveStepState = createLiveStepState();
  ...
  saveReplay(): SaveReplayResponse {
    if (!this.live.replayFrames.length) {
      return { ok: false, error: 'No local match is available to save.' };
    }
    const finalFrame = this.live.replayFrames.at(-1);
    ...
    const replay = {
      // Concealed frames: an agent seat's hand, looks and prompts are already
      // gone before anything is written here.
      visualize: this.live.replayFrames,
      source: {
      ...
      rawVisualize: this.live.rawFrames,
  ...
  private async start(payload: any): Promise<EngineResponse> {
    ...
    this.live = createLiveStepState({ concealedSeats: concealedSeats(playerControls), seats: this.seats() });
    this.live.logs = [{
      id: this.live.logId++,
  ...
  private async select(payload: any): Promise<EngineResponse> {
    const select = this.live.observation?.select;
    if (!select) {
  ...
  private view(): GameView {
    const { observation, logs, dataMaps, actionTimeline } = this.live;
    const view = cabtObservationToGameView(observation, logs, dataMaps, actionTimeline);
    return {
      ...view,
      decision: observation ? projectDecision(observation, this.decisionSeq, dataMaps) : undefined,
      seats: this.seats(),
    };
  }
  ...
  // One bridge response: the human's answered observation, the agent's own
  // observations for the decisions it auto-played, and the human's next
  // observation. buildLiveSteps owns the whole per-response rule (concealment,
  // announce synthesis, phase splitting) so tests can replay it offline.
  private appendSteps(response: BridgeResponse): GameView[] {
    ...
    this.live.seats = this.seats();
    return buildLiveSteps(observations, response.autoActions ?? [], this.live).map((step) => step.view);
  }
  ...
  private invalidateSession(message: string): void {
    ...
    const logId = this.live.logId;
    this.sessionId = '';
    this.decisionSeq = 0;
    this.pendingSequence = [];
    this.decks = [[], []];
    this.live = createLiveStepState({ seats: this.seats() });
    this.live.logs = [...logs, { id: logId, message }];
    this.live.logId = logId + 1;
  }
}
...
// The bridge always runs the local interpreter (PYTHON, default python3) against
// src/engine/cabt_bridge.py. The bridge itself resolves the engine from
// CABT_ENGINE_DIR.
